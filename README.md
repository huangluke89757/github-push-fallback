...
这九条都是实际踩出来的，脚本已经自动处理。但如果你要排查问题，需要理解它们在说什么。
...
在命令行上用 `-f force=true`，会被当成字符串"true"而不是布尔真值，报错：

```
For 'properties/force', "true" is not a boolean. (HTTP 422)
```

布尔和数字类型的参数必须写成 JSON 文件再用 `--input` 传进去。

### ⑥ 全新空仓库要特殊处理

刚建好、还没有任何提交的仓库，**整个 Git Data 接口都是关闭的**（连建 blob 都返回 409）：

```
{"message":"Git Repository is empty.","status":"409"}
```

脚本会自动先用另一条接口落一个"种子文件"把仓库激活。注意这个种子请求**不能指定分支名**——
空仓库还没有分支，指定了会返回 404。

### ⑦ 空目录树会查不到

当远端最后一个文件被删掉后，它的目录树指纹会变成 Git 里那个固定的"空树"值。
**这个对象在 GitHub 上并不存在**，去查它必然 404。脚本会把它当成"没有任何文件"处理。

### ⑧ 默认分支不能写死成 main

`git init` 在部分配置下创建的默认分支叫 `master`，不是 `main`。
如果写死推 `main`，内容会推到错误的分支上，而且**远端那边看起来"推成功了"、不报错**。
脚本默认跟随你本地当前所在的分支。

### ⑨ 只推已提交的内容

脚本推的是**你最后一次提交的内容**，不看工作目录里还没提交的改动。
所以会出现最迷惑的组合：`--dry-run` 报"0 个差异"（确实和远端一致），
但你自己知道刚改了文件——因为改动还没提交。

**改完文件要先 `git add` + `git commit`，再跑这个脚本。** 脚本现在会主动提示
"工作区有 N 处未提交改动，不会被推送"。
